...
class LogisticRegression:
    ...
    validate(
        ...
        validate_y: np.ndarray) -> np.ndarray
        Validate the model.
    ...
    def __init__(
        ...
        if not isinstance(initial_weights, np.ndarray):
            ...
            self._weights = np.zeros((in_dimension, out_dimension)).astype(
                'float32')
        ...
        if not isinstance(initial_bias, np.ndarray):
            ...
            #     'float32')
            self._bias = np.zeros((out_dimension,)).astype('float32')
    ...
    def fit(self,
        ...
        threshold : float
            Threshold for convergence in gradient.
        print_loss : bool
    ...
    def predict(self, data_x: np.ndarray) -> np.ndarray:
        ...
        return self._predict(
            self._process_data_x(data_x)).argmax(1).astype('int8')
...
def main(args):
    ...
    if args.hyperparameter_tuning:
        ...
        regularization_rates = [0.0, 0.5, 1.0, 2.0, 4.0]
        max_epochs = [100, 200, 500, 1000]
        best_acc = 0
    ...
    model.fit(train_x, train_y, args.epochs, args.print_loss)
